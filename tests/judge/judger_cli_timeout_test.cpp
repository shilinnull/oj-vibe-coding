...
TEST(JudgerCliTimeoutTest, ReportsTimeLimitExceededOnInfiniteLoop) {
    std::string src = R"cpp(#include <iostream>
int main(){ volatile int x=0; while(true){ x++; } return 0; }
)cpp";

    json req;
    req["language"] = "cpp";
    req["source_code"] = src;
    req["time_limit_ms"] = 200; // small timeout to trigger TLE
    ...
    EXPECT_EQ(resp["status"].get<std::string>(), "time_limit_exceeded");
    ASSERT_TRUE(resp.contains("results"));
    ASSERT_TRUE(resp["results"].is_array());
    ASSERT_GT(resp["results"].size(), 0);
    EXPECT_EQ(resp["results"][0]["status"].get<std::string>(), "time_limit_exceeded");
}

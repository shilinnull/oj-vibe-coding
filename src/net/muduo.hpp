...
class Connection : public std::enable_shared_from_this<Connection> {
    ...
    private:
        // 修复：安全获取shared_ptr，防止bad_weak_ptr
        PtrConnection SafeSharedFromThis() {
            try {
        ...
        void HandleRead() {
            ...
            if (_in_buffer.ReadAbleSize() > 0) {
                // 修复：检查连接状态 + 安全获取shared_ptr
                if (_statu != DISCONNECTED && _statu != DISCONNECTING) {
                    auto conn_ptr = SafeSharedFromThis();
                    if (conn_ptr) {
                        _message_callback(conn_ptr, &_in_buffer);
                    }
                }
            }
        }
        ...
        void HandleWrite() {
            ...
            if (ret < 0) {
                //发送错误就该关闭连接了，
                if (_in_buffer.ReadAbleSize() > 0 && _statu != DISCONNECTED) {
                    auto conn_ptr = SafeSharedFromThis();
                    if (conn_ptr) {
                        _message_callback(conn_ptr, &_in_buffer);
                    }
        ...
        void HandleEvent() {
            ...
            auto conn_ptr = SafeSharedFromThis();
            if (conn_ptr && _event_callback)  {  _event_callback(conn_ptr); }
        }
        ...
        void EstablishedInLoop() {
            ...
            _channel.EnableRead();
            auto conn_ptr = SafeSharedFromThis();
            if (conn_ptr && _connected_callback) {
                _connected_callback(conn_ptr);
            }
        }
        //这个接口才是实际的释放接口
        void ReleaseInLoop() {
            ...
            auto conn_ptr = SafeSharedFromThis();
            if (conn_ptr) {
                if (_closed_callback) _closed_callback(conn_ptr);
                //移除服务器内部管理的连接信息
                if (_server_closed_callback) _server_closed_callback(conn_ptr);
            }
        }

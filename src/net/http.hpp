...
class HttpRequest {
    public:
        ...
        std::unordered_map<std::string, File> _files;           //multipart 文件字段
    public:
        ...
        File GetFileValue(const std::string &name) const {
...
class HttpContext {
    ...
    private:
        ...
        void ParseMultipartBody() {
            ...
            std::size_t cursor = 0;
            while (true) {
                std::size_t begin = _request._body.find(boundary, cursor);
                if (begin == std::string::npos) {
                    break;
                }
                begin += boundary.size();
                if (begin + 2 <= _request._body.size() && _request._body.compare(begin, 2, "--") == 0) {
                    break;
                }
                if (_request._body.compare(begin, 2, "\r\n") == 0) {
                    begin += 2;
                }
                std::size_t header_end = _request._body.find("\r\n\r\n", begin);
                if (header_end == std::string::npos) {
                    break;
                }
                std::istringstream hss(_request._body.substr(begin, header_end - begin));
                ...
                while (std::getline(hss, header_line)) {
                    ...
                    std::string hk = Trim(header_line.substr(0, colon));
                    std::string hv = Trim(header_line.substr(colon + 1));
                    for (auto &c : hk) {
                        c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
                    }
                    if (hk == "content-disposition") {
                        std::stringstream ss(hv);
                        std::string token;
                        ...
                        file_content_type = hv;
                    }
                }
                std::size_t content_begin = header_end + 4;
                std::size_t next_boundary = _request._body.find(boundary, content_begin);
                if (next_boundary == std::string::npos) {
                    break;
                }
                std::size_t content_end = next_boundary;
                if (content_end >= 2 && _request._body.compare(content_end - 2, 2, "\r\n") == 0) {
                    content_end -= 2;
                }
                if (!name.empty() && !filename.empty()) {
                    _request._files[name] = HttpRequest::File{filename, _request._body.substr(content_begin, content_end - content_begin), file_content_type};
                }
        ...
        bool RecvHttpBody(Buffer *buf) {
            ...
            if (buf->ReadAbleSize() >= real_len) {
                ...
                _recv_statu = RECV_HTTP_OVER;
                if (_request.IsMultipartFormData()) {
                    ParseMultipartBody();
                }
                return true;
            }
    ...
    public:
        ...
        void RecvHttpRequest(Buffer *buf) {
            ...
            switch(_recv_statu) {
                case RECV_HTTP_LINE: RecvHttpLine(buf); [[fallthrough]];
                case RECV_HTTP_HEAD: RecvHttpHead(buf); [[fallthrough]];
                case RECV_HTTP_BODY: RecvHttpBody(buf); break;
                default: break;
            }
            return;
        }
...
class HttpServer {
    private:
        ...
        using ErrorHandlerCallback = std::function<void(const HttpRequest &, HttpResponse *)>;
        using ExceptionHandlerCallback = std::function<void(const HttpRequest &, HttpResponse *, std::exception_ptr)>;
        Handlers _get_route;
        Handlers _post_route;
        Handlers _put_route;
        Handlers _delete_route;
        std::string _basedir; //静态资源根目录
        std::unordered_map<std::string, std::string> _default_headers;
        ErrorHandlerCallback _error_handler;
        ExceptionHandlerCallback _exception_handler;
        TcpServer _server;
    private:
        void RenderDefaultErrorPage(const HttpRequest &req, HttpResponse *rsp) {
            if (_error_handler) {
                _error_handler(req, rsp);
                return;
            }
            //1. 组织一个错误展示页面
            std::string body;
        ...
        void WriteReponse(const PtrConnection &conn, const HttpRequest &req, HttpResponse &rsp) {
            //1. 先完善头部字段
            for (const auto &head : _default_headers) {
                if (rsp.HasHeader(head.first) == false) {
                    rsp.SetHeader(head.first, head.second);
                }
            }
            if (req.Close() == true) {
        ...
        void OnMessage(const PtrConnection &conn, Buffer *buffer) {
            while(buffer->ReadAbleSize() > 0){
                ...
                if (context->RespStatu() >= 400) {
                    //进行错误响应，关闭连接
                    RenderDefaultErrorPage(req, &rsp);//填充一个错误显示页面数据到rsp中
                ...
                try {
                    ...
                    if (_exception_handler) {
                        _exception_handler(req, &rsp, std::current_exception());
                    } else {
                        rsp._statu = 500;
                        RenderDefaultErrorPage(req, &rsp);
                    }
                }
                if (rsp._statu >= 400) {
                    RenderDefaultErrorPage(req, &rsp);
    ...
    public:
        ...
        void SetErrorHandler(const ErrorHandlerCallback &cb) {
            _error_handler = cb;
        }
        void SetExceptionHandler(const ExceptionHandlerCallback &cb) {
            _exception_handler = cb;
        }
